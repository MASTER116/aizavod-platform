...
_FALLBACK_RULES: list[tuple[list[str], str]] = [
    ...
    (
        ["импорт", "ввоз", "растамож", "таможн"],
        "Для ввоза (импорта) ТС в Россию необходимо:\n"
        "1. СБКТС (Свидетельство о безопасности конструкции ТС) — для единичного ТС\n"
        "2. Установка ЭРА-ГЛОНАСС (для категорий M1, N1)\n"
        "3. Оплата утилизационного сбора\n"
        "4. Таможенное оформление (ПТС/ЭПТС)\n"
        "Для серийного импорта — ОТТС (Одобрение типа ТС).\n"
        "Основной регламент: ТР ТС 018/2011 «О безопасности колёсных транспортных средств».",
    ),
    (
        ["сертификат", "какой нужен", "какие документ", "что нужно"],
        "Для сертификации ТС в ЕАЭС нужен один из документов:\n"
        "• ОТТС — для серийного производства/импорта (на модель)\n"
        "• СБКТС — для единичного ТС (на конкретный VIN)\n"
        "• Декларация соответствия — для компонентов\n"
        "Все документы оформляются по ТР ТС 018/2011. "
        "Обращайтесь в аккредитованный орган (НАМИ, ВНИИНМАШ и др.).",
    ),
    (
        ["срок", "долго", "время оформлен"],
        "Сроки оформления:\n"
        "• ОТТС: 3–12 месяцев (включая испытания)\n"
        "• СБКТС: 1–4 недели\n"
        "• Декларация на компонент: 2–6 недель\n"
        "Сроки зависят от категории ТС, объёма испытаний и загрузки лаборатории.",
    ),
]
