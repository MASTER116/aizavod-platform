...
# ─── Промпт отдельской декомпозиции (3-й уровень) ─────────────────────────
...
# ─── Реестр специалистов по отделам ──────────────────────────────────────
...
class Conductor:
    ...
    async def _collect_results(self, original_task: str, tree: dict) -> dict:
        """Собрать результаты со всех уровней и сформировать итоговый отчёт."""
        # Формируем текстовое описание дерева результатов
        results_lines = []
        total = 0
        completed = 0
    ...
    async def orchestrate(self, task: str, depth: int = 3) -> dict:
        ...
        Args:
            task: задача от основателя
            depth: глубина декомпозиции (2 = до отделов, 3 = до специалистов)
        """
        ...
        for dt in director_tasks:
            ...
            # ── Шаг 3: Отдельская декомпозиция (3-й уровень) ──
            if depth >= 3:
                for dept in dept_tasks:
                    ...
                    logger.info(
                        "    %s.%s: %d специалистов",
                        role, dept_code, len(specialist_tasks),
                    )
