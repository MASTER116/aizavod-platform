...
def upgrade() -> None:
    ...
    conn.execute(sa.text("""
        DO $$ BEGIN CREATE TYPE workplanstatus AS ENUM ('backlog','todo','in_progress','review','done','cancelled'); EXCEPTION WHEN duplicate_object THEN null; END $$;
    """))
    conn.execute(sa.text("""
        DO $$ BEGIN CREATE TYPE workplancategory AS ENUM ('development','business','marketing','operations','finance','legal','content','sales','other'); EXCEPTION WHEN duplicate_object THEN null; END $$;
    """))
    ...
    op.create_table(
        ...
        sa.Column('category', sa.String(50), server_default='development', index=True),
        sa.Column('status', sa.String(50), server_default='backlog', index=True),
